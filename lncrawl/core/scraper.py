...
class Scraper(TaskManager, SoupMaker):
    ...
    def __process_request_with_session(self, method: str, url, session, **kwargs):
        method_call = getattr(self.scraper, method)
        assert callable(method_call), f"No request method: {method}"
        ...
        while retry >= 0:
            try:
                ...
                with self.domain_gate(_parsed.hostname):
                    with no_ssl_verification():
                        response: Response = method_call(url, session=session, **kwargs)
                        response.raise_for_status()
                        response.encoding = "utf8"
    ...
    def get_response2(self, session, url, retry=1, timeout=(7, 301), **kwargs) -> Response:
        """Fetch the content and return the response"""
        return self.__process_request_with_session(
            "get2",
            url,
            retry=retry,
            session=session,
    ...
    def get_soup2(self, session, url, headers={}, parser=None, **kwargs) -> BeautifulSoup:
        """Fetch the content and return a BeautifulSoup instance of the page"""
        headers = CaseInsensitiveDict(headers)
        headers.setdefault(
            "Accept",
        ...
        response = self.get_response2(session=session, url=url, **kwargs)
        self.last_soup_url = url
        return self.make_soup(response, parser)
