...
class TruenFull(Crawler):
    has_mtl = True
    base_url = ["https://truyenfull.vn/", "https://truyentr.info/", "https://tytnovel.com"]
    ...
    def read_novel_info(self):
        ...
        elif "//tytnovel.com/" in self.novel_url:
            self.parse_tytnovel_chapters(soup)
        else:
    ...
    def parse_tytnovel_chapters(self, soup: Tag):
        total_page = 1
        pagination = soup.select(".pagination li a")
        ...
        futures.insert(0, self.executor.submit(self.get_soup, self.novel_url))

        for f in futures:
            data = f.result()
            self.parse_all_links(data.select(".list-chapter a"))
